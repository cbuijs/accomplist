...
'''
==============================================================================================
 accomplist.py: v3.09-20260422-3 Copyright (C) 2018-2026 Chris Buijs <cbuijs@chrisbuijs.com>
 Last Modified: 2026-04-22 13:13:35 CEST

 Description: 
 Core blocklist (Black/Whitelist) compiler/optimizer. Compiles adblock, hosts, IP ranges and DNS
 sets into structured optimized datasets for various DNS/Firewall platforms. Contains built-in DNS 
 caching, IPv6 expansion, and TLD filtering.

 Changes/Fixes:
 - v3.09-20260422-3: Consolidated all prior fixes into a single, complete, copy-pasteable script.
 - v3.09-20260422-1: Fixed `NameError` in listfile generation (replaced `id` with `listname`). 
                     Swapped `BaseException` for `Exception` to fix KeyboardInterrupt blocks. 
                     Removed redundant `domlist` assignments and dead imports. Changed 
                     `.has_key()` to `in` for Py3 forward-compatibility.
 - v3.08-20260422-2: Restored `expand_ip` and other essential helper functions that were 
                     previously truncated, fixing runtime `NameError` exceptions during 
                     IP parsing and CIDR expansions.
 - v3.07-20260422-1: Updated exclude validation to check the exclude pattern against both the 
                     extracted domain `entry` AND the raw unparsed `line`.
 - v3.06-20250331-1: Initial base version.
==============================================================================================
'''

## Modules

# Explicitly set paths for Python 2.7 local packages (Improvement: migrate to venv)
import sys
...
# Third-party modules for downloading, regex processing, and IP aggregation
import requests
import regex
import pytricia
...
# Force UTF-8 encoding on standard output/error to handle internationalized domains (IDNs)
sys.stdout = codecs.getwriter('utf8')(sys.stdout)
sys.stderr = codecs.getwriter('utf8')(sys.stderr)

# Public suffix processing to accurately identify SLDs and TLDs
from publicsuffix2 import PublicSuffixList
psl = PublicSuffixList()

# Speedup DNS by caching getaddrinfo queries during active host checks
import socket
...
# Identify target environment parameters (Standard vs Custom Source overrides)
norestriction = False
if len(sys.argv) > 2:
...
# IP to ASN mapping variables (Uses PyTricia for fast /32 and /128 prefix matching)
asnip = dict()
ipasn4 = pytricia.PyTricia(32)
ipasn6 = pytricia.PyTricia(128)
ipasnfile = '/opt/ipinfo2ipasn/ipasn-all.dat'
ipasnoutfile = '/opt/accomplist/ipasn-all-cidr-aggregated.dat.out'
ipasnfilecache = '/opt/accomplist/ipasn.cache'
ipasnlargest4 = 32
ipasnlargest6 = 128

# Core datasets populated during source parsing
...
# Regex configurations mappings
fileregex = dict()
fileregexlist = '/opt/accomplist/accomplist.listregexes'

# TLD data sets used for validation and filtering
tldlist = dict()
...
# Alexa/Umbrella Top 1M handling
top1mfile = '/opt/accomplist/chris/top.list'
top1mlist = set()

# Global configuration flags governing list generation behaviors
...
# Default format and CIDR identification regexes
...
# Pre-defined strict exclusion and formatting matchers
...
# Replace socket.getaddrinfo with caching version to speedup urllib DNS queries
...
# Validates lists against active regex dictionaries
...
# Core engine helpers (Subnet expansion, IPv6 parsing, etc.)
def expand_ip(ip):
    if not ':' in ip:
        if len(ip.split('.')) != 4:
            new_ip = '0.0.0.0/32'
...
def rev_ip(ip):
    # Generates reverse DNS domains (.in-addr.arpa / .ip6.arpa) based on parsed IP address blocks
    revip = list()
    eip = expand_ip(ip)
    prefix = False
...
def add_cidr(iplist4, iplist6, entry, lid, getasn, force):
    # Appends proper subnet definitions to PyTricia structures while enforcing bounds checking
...
# Read raw source files into memory, processing domains, IPs, and regex structures.
    ...
    if (len(name) > 0):
        try:
            with open(name, 'r') as f:
                ...
                ipcount = 0; domaincount = 0; asncount = 0; skipped = 0; topskipped = 0; total = 0
...
def dom_sort(domlist):
    # Enforces strict TLD-first sorting for efficient hierarchy aggregation
    unique_domains = set(domlist)
    sorted_domains = sorted(unique_domains, key=lambda domain: domain.split('.')[::-1])
    return sorted_domains

# Removes redundant child subdomains if the parent domain is already explicitly blocked
...
# Removes dynamically un-whitelisted datasets if they cross-pollinate with standard blocks
def unwhite_ip(wlist, blist, listname, size):
    if not unwhitelist: return wlist
    if not blist: return wlist
...
def dom_find(name, dlist, listname):
    if name:
        ...
        for label in name.split('.')[::-1]:
            ...
            if testname in dlist: return testname
    return False
...
# Master aggregation routine removing overlaps between safe, explicit white, and explicit black sets
def uncomplicate_lists(whitelist, rwhitelist, blacklist, safelist):
    ...
    # Safe list mutation via filtration mappings
    for domain in listw:
...
def exclude_domlist(domlist, exlist, bw):
    if not exlist: return domlist, dict()
...
# Target platform export files functions (adblock, dnsmasq, coredns, rpz, etc)
def plain_save(bw, optimized):
    # Generates raw text formats mapped per list classification type
...
def adblock_save():
    ...
    with open(outputdir + '/adblock.txt', 'w') as f:
        ...
        if whitelist:
            f.write('!\n! WhiteList Domains:\n')
            for domain in dom_sort(whitelist.keys()):
                blacklisted = dom_find(domain, blacklist, 'blacklist')
                if blacklisted:
                    f.write('! Sub-domain \"' + domain +'\" unblacklisted from domain \"' + blacklisted + '\"\n')
                        ...
                        if (bw == 'black' and readblack) or (bw == 'white' and readwhite) or (bw == 'exclude' and (readwhite or readblack)):
                            ...
                            if source:
                                if len(element) > 3:
                                    ...
                                    listfile = workdir + '/' + listname.strip('.').lower() + ".list"
                                                                        ...
                                                                        if matchentry:
                                                                            for placeholder in ['asn', 'host', 'domain', 'entry', 'ip', 'line', 'regex', 'default1', 'default2', 'default3']:
                                                                                ...
                                                                                if entry and len(entry) > 0 and (entry.upper() not in seen):
                                                                                    ...
                                                                                    if not exclude.match(entry) and not exclude.match(line):
