...
'''
===========================================================================================
 accomplist.py: v2.686-20210528-1 Copyright (C) 2018-2021 Chris Buijs <cbuijs@chrisbuijs.com>
===========================================================================================
...
isdomain = regex.compile('(?=^.{1,251}[a-z]$)(^((?!-)[a-z0-9_-]{0,62}[a-z0-9]\.)*(xn--[a-z0-9-]{1,59}|[a-z]{2,63})$)', regex.I)
...
#wwwregex = regex.compile('^(https*|ftps*|www[a-z]*)[0-9]*(-[a-z]+)*[0-9]*\..*$', regex.I)
wwwregex = regex.compile('^(http|ftp|www+)[sz]*[0-9]*\..*$', regex.I)
...
# Read file/list
def read_lists(lid, name, regexlist, iplist4, iplist6, domainlist, asnlist, safelist, safewlist, force, bw, getasn, maxnum, topcheck, wwwstrip):
                        ...
                        if entry.upper() not in seen: # Unduplicate
                            ...
                            if (isregex.match(entry)): # and (entry.upper() not in seen):
                                ...
                                if negate is False and cleanregex.find('(?!') > 0:
                                    log_info('INFO: Ignoring/Skipping NEGATE line/regex \"' + entry + '\"')
                                    skipped += 1

                                else:
                                    if cleanregex.find('(?!') > 0:
                                        log_info('INFO: Using NEGATE line/regex \"' + entry + '\"')
                            ...
                            # It is an IP
                                ...
                                for dom in tldlist:
                                    ...
                                    if (isdomain.match(domain)):
                                        if (debug >= 2): log_info(lid + ': Added TLD entry \"' + entry + '\" -> \"' + domain + '\"')
                                        domainlist[domain] = 'TLD-' + lid
                                        domaincount += 1
                                    else:
                                        if (debug >= 3): log_info(lid + ': Skipped invalid line \"' + entry + '\"')
                                        skipped += 1
                                ...
                                if wwwstrip:
                                    wwwcount = 0
                                    newdomain = False
                                    while wwwregex.match(domain) and domain.count('.') > 1:
                                        wwwcount += 1
                                        label = domain.split('.')[0]
                                        newdomain = '.'.join(domain.split('.')[1:])
                                        if (debug >= 2): log_info('Stripped \"' + label + '\" (' + str(wwwcount) + ') from \"' + domain + '\" -> \"' + newdomain + '\"')
                                        domain = newdomain

                                    newdomain = regex.sub('^(0\.0\.0\.0|127\.0\.0\.1)\.*', '', domain)
                                    if newdomain != domain:
					if (debug >= 2): log_info('Stripped IP-Address from \"' + domain + '\" -> \"' + newdomain + '\"')
                                        domain = newdomain
                                ...
                                if domain:
                                    if tldlist and (not safed) and (not force):
                                        ...
                                        if not tld in tldlist:
                                            if (debug >= 3): log_info('Skipped DOMAIN \"' + domain + '\", TLD (' + tld + ') does not exist')
                                            domain = False
                                            addtolist[tld] = 'Invalid-TLD-' + lid
                                            skipped += 1

                                    if domain and topcheck and top1mlist and domain.count('.') > 0:
                                        istop1m = dom_find(domain, top1mlist, 'top1mlist')
                                        if istop1m:
                                            if (debug >= 2): log_info('Top: \"' + domain + '\" -> \"' + istop1m + '\"')
                                        else:
                                            if (debug >= 2): log_info('Skipped \"' + domain + '\" - Not in TOP list')
...
def add_cidr(iplist4, iplist6, entry, lid, getasn, force):
    ...
    bits = 32
    cidr = '{0}'.format(IP(cidr, make_net=True).strNormal(1))
    if cidr.find('/') > -1:
    ...
    if expand_ip(entry.split('/')[0]) != expand_ip(cidr.split('/')[0]):
        log_err(lid + ': Fixed CIDR from \"' + entry + '\" to \"' + cidr + '\"')

    # Safety against to large subnets blocking
    if (ipv6 is False and bits < 8) or (ipv6 is True and bits < 20):
        if force:
            log_err(lid + ': Skipped invalid line/ip-address \"' + entry + '\" - too large (/' + str(bits) + '), max allowed: ipv4=/8 and ipv6=/20')
            return False
        else:
            log_err(lid + ': Allowed/Forced invalid line/ip-address \"' + entry + '\" - too large (/' + str(bits) + '), max allowed: ipv4=/8 and ipv6=/20')
        ...
        if iplist.has_key(cidr):
            if iplist[cidr].find(lid) == -1:
                ...
                try:
                    ...
                    log_err(lid + ': Skipped invalid line/ip-address \"' + entry + '\" - ' + str(err))
                    return False
        else:
            try:
...
def make_top(domlst):
    if top1mlist:
        if (debug >= 2): log_info('\nMaking TOP-N Blacklist ...')
        newdomlst = dict()
        for domain in sorted(domlst.keys()):
            if dom_find(domain, top1mlist, 'top1mlist'):
                newdomlst[domain] = domlst[domain]

        if (debug >= 2): log_info('TOP-N Blacklist has ' + str(len(newdomlst)) + ' domains (Full is ' + str(len(domlst)) + ')')
...
#    keyname = hash(listname + ':' + name)
#    if keyname in dom_cache:
#        return dom_cache[keyname]

#    testname = name
#    while True:
#        if testname in dlist:
#            dom_cache[keyname] = testname
#            return testname
#        elif testname.find('.') == -1:
#            dom_cache[keyname] = False
#            return False
#        else:
    ...
    for domain in domlist.keys():
        ...
        if found:
            ...
            if ebw == 'exclude' or ebw != bw:
                if (debug >= 3): log_info(bw + 'list: Excluded \"' + domain + '\" - Matched against ' + ebw + 'listed \"' + found + '\"')
            else:
...
def plain_save(bw, optimized):
    ...
    if len(domlist) > 0:
        with open(outputdir + fprefix + '.domain.list', 'w') as f:
            for domain in sorted(domlist.keys()):
                f.write(domain)
                f.write('\n')

        if bw == 'black' and len(topdoms) > 0:
            with open(outputdir + fprefix + '.top-n.domain.list', 'w') as f:
                for domain in sorted(topdoms.keys()):
                    if dom_find(domain, top1mlist, 'top1mlist'):
                        f.write('{0}'.format(domain.strip('.')))
                        f.write('\n')

            #for ip in iplist4.keys():
            #    arpa = rev_ip(ip, False)
            #    if arpa:
            #        f.write(arpa)
            #        f.write('\n')
...
def routedns_save(optimized):
    ...
    if len(whitelist) > 0:
        with open(outputdir + '/routedns.allowlist' + pref + '.domain.list', 'w') as f:
            for domain in sorted(whitelist.keys()):
                f.write('.{0}'.format(domain.strip('.')))
                f.write('\n')
    ...
    if len(blacklist) > 0:
        with open(outputdir + '/routedns.blocklist' + pref + '.domain.list', 'w') as f:
            for domain in sorted(blacklist.keys()):
                f.write('.{0}'.format(domain.strip('.')))
                f.write('\n')
...
def ls_save(bw):
    ...
    nor = len(domlist) + len(iplist4) + len(iplist6)
    if nor > 1000000:
        log_err('More then 10000 (' + str(nor) + ') rules, not generating Little-Snitch files!')
        return False

    with open(outputdir + '/litte-snitch.' + bw + '.lsrules', 'w') as f:
        f.write('{\n')
        f.write('\t\"description\" : \"Accomplist ' + bw + 'list (' + str(int(time.time())) + '). See: https://github.com/cbuijs/accomplist\",\n')
...
# Save HostsFile
def hosts_save():
    log_info('\nCreating plain.hosts in ' + outputdir)
    if blacklist:
...
def adblock_save():
    ...
    with open(outputdir + '/adblock.txt', 'w') as f:
        ...
        f.write('! Title: Accomplist \"' + sourcename.title() + '\" AdBlock List\n')
...
def deugniets_save(bw, domlist, ip4list, ip6list, rxlist):
    ...
    with open(outputdir + '/' + filename, 'w') as f:
        ...
        if domlist:
            ...
            for line in sorted(domlist.keys()):
                f.write(line + '\n')
    ...
    with open(outputdir + '/unbound-filter.conf', 'w') as f:
        ...
        if cblacklist4:
            ...
            for cidr in cblacklist4.keys():
                ...
                #arpa = rev_ip(cidr, False)
                #if arpa:
                #    f.write('\tlocal-zone: \"' + arpa + '\" always_nxdomain\t# ' + cblacklist4[cidr] + '\n')
        ...
        if cblacklist6:
            ...
            for cidr in cblacklist6.keys():
                ...
                #arpa = rev_ip(cidr, False)
                #if arpa:
                #    f.write('\tlocal-zone: \"' + arpa + '\" always_nxdomain\t# ' + cblacklist6[cidr] + '\n')
	...
	if whitelist:
        ...
        if cblacklist4:
            ...
            for cidr in cblacklist4.keys():
                ...
                #arpa = rev_ip(cidr, False)
                #if arpa:
                #    f.write('server=/' + arpa + '/\t#  ' + cblacklist4[cidr] + '\n')
...
def rpz_save(bw):
	...
	lname = "WHITELIST"
    	...
    	file = outputdir + '/db.' + bw + '.top-n.rpz'
	fulllist = blacklist
	domlist = topdoms
        iplist4 = cblacklist4
        iplist6 = cblacklist6
    ...
    with open(file, 'w') as f:
        ...
        f.write(';\n; --- TLDS ---\n')
        for domain in sorted(fulllist.keys()):
            if '.' not in domain:
    ...
    if prefix:
        ...
        if ipregex4.search(eip):
            if prefix in (8, 16, 24, 32):
                revip = '.'.join(eip.split('.')[0:prefix / 8][::-1]) + '.in-addr.arpa'
            elif delimiter:
                octs = eip.split('.')[::-1]
...
if __name__ == "__main__":
    ...
    if age and age < maxlistage:
        ...
        read_lists('saved-whitelist', whitesave, rwhitelist, cwhitelist4, cwhitelist6, whitelist, asnwhitelist, safewhitelist, safeunwhitelist, True, 'white', False, False, False, False)
        readwhite = False

    age = file_exist(blacksave)
    if age and age < maxlistage:
        log_info('Using Black-Savelist, not expired yet (' + str(age) + '/' + str(maxlistage) + ')')
        read_lists('saved-blacklist', blacksave, rblacklist, cblacklist4, cblacklist6, blacklist, asnblacklist, safeblacklist, False, True, 'black', False, False, False, False)
        readblack = False
            ...
            for line in f:
                ...
                if not (entry.startswith("#")) and not (len(entry) == 0):
                    ...
                    if len(element) > 2:
                        ...
                        if bw.endswith('!'):
                            bw = bw.rstrip('!')
                            if (debug > 1): log_info('The ' + bw + 'list \"' + listname + '\" will be FORCED!')
                            forced = True
                        ...
                        if bw.endswith('*'):
                            bw = bw.rstrip('*')
                            if (debug > 1): log_info('Retrieving ASNs for ' + bw + 'list \"' + listname + '\"')
                            getasn = True

                        topcheck = False
                        if bw.endswith('^'):
                            bw = bw.rstrip('^')
                            if (debug > 1): log_info('Only use TOP domain of ' + bw + 'list \"' + listname + '\"')
                            topcheck = True

                        stripwww = True
                        if bw.endswith('('):
                            bw = bw.rstrip('(')
                            if (debug > 1): log_info('Do not strip WWW labels from ' + bw + 'list \"' + listname + '\"')
                            stripwww = False
                        ...
                        if (bw == 'black' and readblack) or (bw == 'white' and readwhite) or (bw == 'exclude' and (readwhite or readblack)):
                                                            ...
                                                            if line and len(line) > 0:
                                                                for rx in regex.split('\s*,\s*', fregex):
                                                                    if rx in fileregex:
                                                                        ...
                                                                        if matchentry:
                                                                            for placeholder in ['asn', 'host', 'domain', 'entry', 'ip', 'line', 'regex']:
                                                                                try:
                            ...
                            if file_exist(listfile) >= 0:
                                if bw == 'black':
                                    read_lists(listname, listfile, rblacklist, cblacklist4, cblacklist6, blacklist, asnblacklist, safeblacklist, False, force, bw, getasn, maxnum, topcheck, stripwww)
                                elif bw == 'white':
                                    read_lists(listname, listfile, rwhitelist, cwhitelist4, cwhitelist6, whitelist, asnwhitelist, safewhitelist, safeunwhitelist, force, bw, getasn, maxnum, topcheck, stripwww)
                                elif bw == 'exclude':
                                    excount = 0
                                    try:
    ...
    if excludelist:
        ...
        for dom in excludelist.keys():
            bw = excludelist.get(dom, 'exclude')
            if bw == 'white':
                log_info('Added exclude domain \"' + dom + '\" to whitelist')
                whitelist[dom] = 'Exclude'
            elif bw == 'black':
                log_info('Added exclude domain \"' + dom + '\" to Blacklist')
                blacklist[dom] = 'Exclude'

        # Remove exclude entries from lists
        whitelist = exclude_domlist(whitelist, excludelist, 'white')
        blacklist = exclude_domlist(blacklist, excludelist, 'black')
